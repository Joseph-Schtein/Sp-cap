            ...
            if len(value) > 0:
                ...
                if key == elective_course_list[j].get_name():
                    if len(possibles) == 1:
                            ...
                            if check.count(0) == len(check):    # In case there isn't a tie between student bids
                                for stu in range(len(student_object_try)):
                                    if elective_course_list[j].get_capacity() > 0 and check_overlap(student_object_try[stu], elective_course_list[j]):
                                        ...
                                        s = Student.objects.get(student_id=student_object_try[stu].get_id())
                                        c = Course.objects.get(course_id=elective_course_list[j].get_id())
                                        Result.objects.create(course=c, student=s, selected=True)
                        ...
                        failed_to_enroll = [False for i in range(len(possibles))]
                        ...
                        if total_capacity >= len(try_to_enroll):
                            for need_to in range(len(try_to_enroll)):
                                if check_overlap(student_object_try[need_to], sorted_possibles[option % len(possibles)]):
                                    sorted_possibles[option % len(possibles)].student_enrollment(try_to_enroll[need_to],
                                                                           student_object_try[need_to])
                                    student_object_try[need_to].got_enrolled(sorted_possibles[option % len(possibles)].get_name())
                                    option += 1
