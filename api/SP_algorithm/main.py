...
def order_course_data(course_list):
    ...
    for dic in course_list:
        ...
        for dic2 in dic['courses']:
            ...
            tmp = Course(id, name, capacity, start, end, semester, day, lecturer)
            possible_list.append(tmp)

        for co in possible_list:
            course_list_output.append(co)

        new_group = Course_group(name, office, possible_list)
        group_course_list.append(new_group)
        possible_list.clear()

    return group_course_list, course_list_output
...
def order_student_data(raw_student_list, raw_rank_list, course_list):
    indexed_enrollment = {}
    student_list = []
    ...
    for dic in raw_student_list:
        tmp = deepcopy(indexed_enrollment)
        id = dic['student_id']
        need_to_enroll = dic['amount_elective']
        office = dic['office']
        for dic2 in dic['courses']:
            for i in course_list:
                if i.get_id() == dic2['course_id']:
                    tmp['course_id'] = 1




        student_list.append(Student(id, need_to_enroll, office, [], indexed_enrollment))
...
def main(raw_student_list, raw_course_list, raw_rank_list):
    ...
    course_group_list, course_list = order_course_data(raw_course_list)
    overlap_course(course_list)
    for i in course_list:
        i.to_string()
        print()

    order_student_data(raw_student_list, raw_rank_list, course_list)
